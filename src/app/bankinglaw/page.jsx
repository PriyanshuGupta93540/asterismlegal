...
useEffect(() => {
  const links = document.querySelectorAll('.nav-linkservice');
  links.forEach(link => {
    if (link.getAttribute('href') === window.location.pathname) {
      link.classList.add('activeservice');
    } else {
      link.classList.remove('activeservice');
    }
  });
}, []);
...
<div className='absolute top-96 left-100'>
         <div className='text-overlay mt-36 w-99 h-24 bg-white z-0 flex pt-4 relative ml-24'>
                ...
                <div className='text-black mt-2 '>
                    <h1 className="text-5xl font-sans ml-2">BANKING LAW</h1>
      ...
      <div className='flex'>
       <div className='flex w-96 h-72 p-8 text-black ml-16 mt-16 font-semibold'>
           ...
           <li className='border h-12 border-slate-500 pl-2 pt-2 nav-linkservice'>
                <Link href=
                '/bankruptcyInsolvencylaw'>BANKRUPTCY & INSOLVENCY LAW</Link>
            </li>
            <li className='border h-12 border-slate-500 pl-2 pt-2 nav-linkservice'>
