...
const Testimonials = () => {
  ...
  return (
    <div className='flex w-3/4 h-99 ml-36 about-testim'>
      <div className="relative t w-36 py-2 md:py-24 cus md:w-1/2 flex flex-col item-center justify-center qual qual-test">
        <div className="absolute about-testim-what top-0 left-0 z-10 grid-indigo w-16 h-16 md:w-40 md:h-40 md:ml-20 md:mt-24"></div>
        <div className="relative text-2xl  test-span md:text-5xl py-2 px-6 md:py-6 md:px-1 md:w-64 md:mx-auto text-indigo-100 font-semibold leading-tight tracking-tight mb-0 z-20">
        ...
        <div className="absolute right-0 bottom-0 mr-4 mb-4 hidden md:block about-circle">
          <button
      ...
      <div className="bg-gray-100 md:w-1/2 test-bg">
        <div className="flex flex-col h-full relative">
          ...
          <div className="h-72 relative z-10 test-content-div">
            <p className="text-gray-600 serif test-content font-normal italic px-6 py-6 md:px-16 md:py-10 text-xl md:text-xl">
              {testimonials[testimonialActive - 1].content}
            </p>
          </div>
          <div className="flex my-3 justify-center items-center rounde about-circle-main">
            {testimonials.map((item, index) => (
              <button
                key={index}
                onClick={() => setTestimonialActive(index + 1)}
                className={`text-center font-bold shadow-xs focus:outline-none focus:shadow-outline rounded-full ${testimonialActive !== index + 1 ? 'h-12 w-12 opacity-25 bg-yllow-300  mr-2 text-gray-600' : 'h-16 w-16 opacity-100 bg-yellow-500 text-white test-name'
                }`}
              >
                {item.name}
              </button>
          ...
          <div className="flex justify-center px-6 pt-2 pb-6 md:py-6 about-circle">
